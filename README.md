...
MeteoSwiss uses the [ICON-CH1-EPS and ICON-CH2-EPS models](https://www.meteoswiss.admin.ch/weather/warning-and-forecasting-systems/icon-forecasting-systems.html) to forecast weather in Switzerland up to five days ahead. Both models apply [ensemble data assimilation](https://www.meteoswiss.admin.ch/weather/warning-and-forecasting-systems/icon-forecasting-systems/ensemble-data-assimilation.html).
...
| **Horizontal Grid**                   | Native icosahedral           | Native icosahedral           |
...
Users can view the list of currently available parameters, along with their associated metadata, in the CSV files located within the collection assets of each respective collection.
- Collection 1 Parameters (CSV)
- Collection 2 Parameters (CSV)

Users can access forecast model data from the last **24 hours**. Data older than this is no longer available.

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<Tabs queryString="data-download">
  <TabItem value="via-python-api" label="User-friendly Python API">
  - "more description about get_from_ogd(), download_from_ogd()
  - Notebook examples:  From Retrieval to Visualization
  ...
  <TabItem value="via-stac-api" label="HTTP requests to the STAC API">
...
4. Once the static GRIB file is downloaded, ensure that the `uuidOfHGrid` (Universally Unique Identifier) key in the data file matches the one in the static horizontal file.
  ...
  <TabItem value="via-catalog" label="Open Data catalog">
  Add text that file item can be downloaded though the catalog
  </TabItem>
</Tabs>

### Data structure
<--- one file is one ref time, one var,...>
![alt text](Images/ogd_data_structure.drawio.png)

<---- in one file we have the full domain and here below a description of the domain --->
...
<- Add a note about easier to read in python with meteodata-lab)
